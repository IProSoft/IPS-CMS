...
function factory()
{
	...
	*/
	function CanvasEditor( scope, options, callback ) {
	// coerce CanvasEditor() without new, to be new CanvasEditor()
		if ( !( this instanceof CanvasEditor ) ) {
			return new CanvasEditor( scope, options, callback );
		}
		this.options = $.extend( {}, options );

		return this.getInstance(scope, callback);
	}
	CanvasEditor.prototype = {};

	CanvasEditor.prototype.getInstance = function( scope, callback ){
	  
		this.modules = this.getModules(scope);
	  
		this.getModuleRequire( this.modules )( scope + "/src/carota" );
		
		return window[scope]( callback )
	};

	CanvasEditor.prototype.log = {
		log: function(){
		...
		get: function(){
			console.clear();
			console.log(this.logCache)
		}
	}
	CanvasEditor.prototype.getModules = function( scope ){ 
						...
						if (typeof window !== 'undefined' && window.document) {
							...
							window[scope] = function( options ){
								...
								var bundle = {
									...
									rect: rect,
									format: format,
									hash: IpsApp._rand( 10, 'string' )
								};
								
								bundle.format.set( options, bundle.format.hash );
						...
						var prototype = node.derive({
							...
							layout: function() {
								...
								if (!this.frame) {
									...
									this.select(next.start, next.end, false);/* IPS added false */
								}
							},
		 ...
		 */
							...
							isSaving: function(word) {
								return typeof this.saving !== 'undefined' ? this.saving : false;
							},
								...
								if (node) {
									if (node.block && ordinal > 0) {
										...
										if (nodeBefore.newLine) {
											...
											b = rect(lineBounds.l, lineBounds.b, 3, newLineBounds.h);
										} else {
											b = nodeBefore.bounds();
											b = rect(b.r, b.t, 3, b.h);
										}
									} else {
										b = node.bounds();
										if (b.h) {
											b = rect(b.l, b.t, 3, b.h);
										} else {
											b = rect(b.l, b.t, b.w, 3);
										}
									}
									return b;
								}
					...
					"editor.js": function (exports, module, require) {
						...
						exports.intrval = function() {
							$__.clear();
							var editor = exports.element.querySelector('.carotaEditorCanvas');
						...
						setInterval(exports.intrval, 200 )
						...
						exports.create = function(element) {
							...
							var paint = function() {
								...
								if( format.get( 'bgFillColor' ) )
								{
									ctx.fillStyle = format.get( 'bgFillColor' );
					...
					"frame.js": function (exports, module, require) {
						...
						var prototype = node.derive({
							...
							draw: function(ctx, viewPort) {
								$__.log( 'frame.js draw', {viewPort:viewPort, lines:this.lines})
						...
						exports = module.exports = function(left, top, width, ordinal, parent,
							...
							var wrapper = wrap(left, top + format.get( 'topPadding' ) /** IPS **/, width, ordinal, frame, includeTerminator, initialAscent, initialDescent);
							
							var length = 0, height = 0;
							return function(emit, word) {
								if (wrapper(function(line) {
									if (typeof line === 'number') {
										height = line - format.get( 'baselineRatio' ); /** IPS **/
									} else {
					...
					"line.js": function (exports, module, require) {
						...
						module.exports = function(doc, left, width, baseline, ascent, descent, words, ordinal) {
							...
							if (actualWidth < width) {
								switch (align) {
									case 'left':
										x = format.get( 'padding' );
									break;
									case 'right':
										x = width - actualWidth - format.get( 'padding' );
									break;
									case 'justify':
					...
					"node.js": function (exports, module, require) {
						...
						exports.prototype = {
							...
							draw: function(ctx, viewPort) {
								$__.log( 'node.js draw', {viewPort:viewPort, children:this.children()});
					...
					"part.js": function (exports, module, require) {
						...
						var prototype = {
							draw: function(ctx, x, y) {
								$__.log( 'part.js draw', {x:x,y:y, run: this.run} );
					...
					"range.js": function (exports, module, require) {
						...
						Range.prototype.getFormatting = function() {
							...
							return per(range.runs, range).reduce(runs.merge).last() || format.getFormatting();
						};
					...
					"format.js": function (exports, module, require) {
						...
						exports.get = function( key ) {
							return exports.options[key]
						};
						exports.getFormatting = function() {
					...
					"text.js": function (exports, module, require) {
						...
						var getFontString = exports.getFontString = function(run) {
							...
							var size = (run && run.size) || format.get( 'size' );
							...
							return (run && run.italic ? 'italic ' : '') +
								   (run && run.bold ? 'bold ' : '') + ' ' +
									size + 'pt ' +
								  ((run && run.font) || format.get( 'font' ));
						};

						/*  Applies the style of a run to the canvas context
						 */
						exports.applyRunStyle = function(ctx, run) {
							ctx.fillStyle = (run && run.color) || format.get( 'color' );
							ctx.shadowColor = format.get( 'shadowColor' );
						...
						exports.getRunStyle = function(run) {
							var parts = [
							  ...
							  '; color: ', ((run && run.color) || format.get( 'color' ))
							];
						...
						var measureText = exports.measureText = function(text, style) {
							...
							try {
								...
								result.height = (block.offsetTop - span.offsetTop) + 5;/** IPS **/
						...
						exports.draw = function(ctx, str, formatting, left, baseline, width, ascent, descent) {
							...
							if( str !== '\n' && str !== '' )
							{
								if( format.get( 'border' ) )
								{
									...
									ctx.fillStyle = format.get( 'border_color' );
									ctx.beginPath();
									ctx.rect(
										left - 10, 
										baseline - ascent + 1, 
								...
								if( format.get( 'shadow' ) )
								{
									/** IPS **/
									
									ctx.shadowOffsetX = ctx.canvas.width + left;
									ctx.shadowOffsetY = 0;
									ctx.shadowBlur = format.get( 'shadowBlur' );
									
									var shadow_left = -ctx.canvas.width;
									
									ctx.fillText( str, 2 * shadow_left, baseline );
									
									for ( var i = 0; i < 5; i++ )
									{
										ctx.fillText( str, shadow_left, baseline );
									}
								}
							}
					...
					"word.js": function (exports, module, require) {
						...
						var prototype = {
							...
							draw: function(ctx, x, y) {
								$__.log( 'word.js draw', {x:x,y:y,text:this.text} );
  ...
  CanvasEditor.prototype.getModuleRequire = function () {
		...
		return getRequire($this.modules, [], '');
	};
  ...
  if ( $ ) {
    $.fn.CanvasEditor = function( scope, options, callback ) {
      return new CanvasEditor( scope, options, callback );
    };
  }
