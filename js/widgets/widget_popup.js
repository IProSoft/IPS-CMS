(function ($) {
	$.fn.popupIPS = function (options) {
		...
		defaultOpt.opacity = '0.' + defaultOpt.opacity;

		setTimeout( function () {
			
			IpsApp._FB.afterInit( function(){
				FB.getLoginStatus(function(response) {
					if (response.status === 'connected' && ips_config.fanpage_id.toString().length > 8 ) {
						FB.api( 'me/likes/' + ips_config.fanpage_id.toString() + '?access_token=' + response.authResponse.accessToken, function(response) {
			...
			IpsApp._ajax( '/ajax/get_template/?template_name=widget_popup_content', { 
				template_name: '/widgets/widget_popup_content',
				like_box : defaultOpt.box,
				url : defaultOpt.url,
				close_timer : ( defaultOpt.close ? defaultOpt.close_timer : false )
			}, 'POST', 'json', false, function( response ){
				
				$(document.body).append( response.content );
			
				var ipsPopup = $.cookie('ips_popup');
				var ipsPopupTimer = $.cookie('ips_popup_timer');

				if ( ipsPopup != 'true' && ipsPopupTimer != 'true' )
				{
					$("#ips-popup-wrapper").css({
					...
					$("#ips-popup").css({
						"top": "180px",
						"left": document.documentElement.clientWidth / 2 - $("#ips-popup").width() / 2
					}).fadeIn("slow");
					
					if (defaultOpt.timeout == '0') {
						$("#ips-popup-timer-count").hide()
					} else {
						$('#ips-countdown').countdown({
							until: '+' + defaultOpt.timeout + 's',
							/* onExpiry: saveLikePopup */
						})
					};
					
					if (defaultOpt.delay != 0) {
						$.cookie('ips_popup_timer', 'true', { expires: defaultOpt.delay+'s', path: '/' });
					};
					
					IpsApp._FB.afterInit( function(){
						FB.Event.subscribe('edge.create', function ( href ) {
							console.log(href);
							if( $("#ips-popup").is(':visible') )
							{
								saveLikePopup(true);
							}
						});
					...
					$('#ips-popup-wrapper').on( 'click', function(){
						saveLikePopup( defaultOpt.close_timer );
					});
				}
			} );
		...
		}, ( defaultOpt.delay * 1000 ) );
		
		return true
	}
})(jQuery);
function saveLikePopup( action )
{
	$("#ips-popup-wrapper,#ips-popup").fadeOut("slow");

	var timer = ( action == 0 || action == true || typeof action == 'undefined' ? '1y' : action + 's' );
	
	$.cookie( 'ips_popup', 'true', { 
		expires: timer, 
		path: '/'
	} );
