$(document).ready(function () {
		...
		$.fn.FBIPS = function (method) {
			var methods = {
				init: function (options) {
					this.FBIPS.config = $.extend({},
					this.FBIPS.defaults, options);
				...
				checkPublishPrivileges: function () {
					if (this.is(':checked')) {
						$.cookie('activityPublish', 'true', {
							path: '/'
						});
						this.FBIPS('publishPrivileges', (1), ('Włączone'))
					} else {
						$.cookie('activityPublish', 'false', {
							path: '/'
						});
				...
				connectedUserActions: function () {
					...
					userAppFriends();
					$(document).on("click", IPSDIV + " #user-friends .inviteFriend, " + IPSDIV + " .add-friend, .inviteFriend", function () {
				...
				loadUserActivity: function () {
					...
					FB.api('/me/news.reads?date_format=d-m-Y%20H:i&limit=50', function (response) {
						...
						for (i in response.data) {
							crt = response.data[i];
							content.push('<li id="act-' + crt.id + '" class="activity-title"><ul><li class="title">' + crt.data.article.title + '</li><li class="actions"><a href="#" class="user-activity-delete" data-object-id="' + crt.id + '">'+ips_i18n.__( 'js_app_delete' )+'</a></li></ul></li>')
						}
						container.append(content.join('\n'))
					})
				},
				publishPrivileges: function (op, txt) {
					$(IPSDIV + " #user-friends").animate({
						opacity: op
					});
				...
				appendUserActivityLinks: function (userData) {
					...
					if (userData.data.length > 0) {
						for (i in userData.data) {
							content.push("<li id='" + userData.uid + "'>" + ips_i18n.__( 'js_app_file' ) + " <a href='" + userData.data[i].data.article.url + "' alt='" + userData.data[i].data.article.title + "'>" + userData.data[i].data.article.title + "</a></li>")
						}
					} else {
						content.push( ips_i18n.__( 'js_app_none_viewed' ) )
					}
				...
				appRequestUsers: function () {
					FB.ui({
						method: 'apprequests',
						dispay: 'popup',
						message: ips_i18n.__( 'js_app_request' ),
	...
	function userAppFriendsTimeline(users) {
		...
		for (i in users) {
			batchQ.push({
				"method": "GET",
				"relative_url": "/" + users[i].uid + "/news.reads?date_format=d-m-Y H:i&limit=60"
			})
		}
		FB.api("/", "POST", {
			batch: batchQ
		...
		function (response) {
			...
			for (i in response) {
				userFriendsActivity.push($.extend(users[i], $.parseJSON(response[i].body)))
			};
	...
	function appendUsersData(u) {
		...
		$("#user-friends .preload").fadeOut(function () {
			...
			for (i = 0; i < anonymousCount; i++) {
				appendUsers.push('<li><img src="http://profile.ak.fbcdn.net/static-ak/rsrc.php/v2/yo/r/UlIqmHJn-SK.gif" alt="'+ips_i18n.__( 'js_app_add_freind' )+'" class="user-avatar add-friend"/></li>')
			}
			...
			setTimeout(function () {
				...
				function (response) {
					if (!response || response.error) {
						...
						alert( ips_i18n.__( 'js_app_saved_sction' ) )
					}
				})
			},
			10000)
...
'widget_friends_1' => 'Znajomi ',
	'widget_friends_2' => 'zaproś więcej znajomych',
	'widget_friends_3' => 'Trwa ładowanie...',
	'widget_friends_4' => 'Włącz aplikację aby zobaczyć aktywność znajomych.',
	'widget_friends_5' => 'Twoje konto ',
	'widget_friends_6' => 'historia aktywności',
	'widget_friends_7' => 'Włączone',
	'widget_friends_8' => 'Ostatnie publikacje:',
	'widget_friends_9' => 'Dowiedz się <strong>co oglądają Twoi znajomi</strong> i udostępnij im swoją aktywność, jedyne co musisz zrobić to kliknąć ikonę "<strong><a href="/connect/facebook/">Zaloguj się</a></strong>"!',