...
class RainTPL{
	...
	// -------------------------
		...
		static $checked_tpl = array();
		
		static $templatesUsed = array('calls' => array());
	// -------------------------
	...
	// -------------------------
		...
		public static $cache = true;		// static cache enabled / disabled
                
		protected static $config_hash = array();   // takes all the config to create the md5 of the file
	// -------------------------
	...
	static function configure( $setting, $value = null )
	{
		...
		if( is_array( $setting ) )
		{
			...
			self::$config_hash = substr( md5( serialize( $setting ) . ( defined( 'IPS_LNG' ) ? IPS_LNG : '' ) ) , 0, 15);
		}
	...
	protected function config_hash()
	{
	...
	protected function check_template( $tpl_name )
	{
		...
		if( !isset( self::$checked_tpl[ $tpl_name . 'checked' ] ) || self::$checked_tpl[ $tpl_name . 'checked' ] == false )
		{
			/* ips_log( $tpl_name . "\n" ); */
			$tpl_basename                   = ltrim( $tpl_name,  '/'  );																				// template basename
			$tpl_basedir                    = strpos( $tpl_name, '/' ) ? dirname( $tpl_name ) . '/' : null;												// template basedirectory
			$tpl_dir                        = ( strpos( $tpl_name, '__' ) ? ABS_PATH . '/templates/' : self::$tpl_dir ) . ltrim( $tpl_basedir,  '/' );	// template directory
			$this->tpl['tpl_filename']      = $tpl_dir . $tpl_basename . ( strpos( $tpl_basename, self::$tpl_ext ) == true ? '' : self::$tpl_ext);		// template filename
			$temp_compiled_filename         = self::$cache_dir . basename( $tpl_name ) . "." . self::config_hash();
			...
			{
				if( strpos( $this->tpl['tpl_filename'], '__common' ) === false )
				{
					return $this->check_template( '/__common/' . $tpl_basename );
				}
				
				$e = new RainTpl_NotFoundException( 'Template '. $tpl_basename .' not found!'.serialize($this->tpl) );
			...
			//echo filemtime($this->tpl['compiled_filename']).'<br>'.filemtime( $this->tpl['tpl_filename'] );exit;
			if( self::$cache && file_exists( $this->tpl['compiled_filename'] ) && filemtime( $this->tpl['compiled_filename'] ) >= filemtime( $this->tpl['tpl_filename'] ) )
			{
				//Optimized way
				$data = file_get_contents( $this->tpl['compiled_filename'], false, null, 0, 1 );
				if( !empty( $data ) )
				{
					self::$checked_tpl[ $tpl_name . 'checked' ] = $this->tpl;
					$this->templateDebug( $tpl_basename, 'cached' );
					
					return true;
				}
			}
			
			$this->templateDebug( $tpl_basename, 'normal' );
			
			$this->compileFile( $tpl_basename, $tpl_basedir, $this->tpl['tpl_filename'], self::$cache_dir, $this->tpl['compiled_filename'] );
			
			self::$checked_tpl[ $tpl_name . 'checked' ] = false;
			
			return false;	
		}
		else
		{
			$this->tpl = self::$checked_tpl[ $tpl_name . 'checked' ];
			return true;
		}
	}

	protected function templateDebug( $tpl_basename, $type )
	{
		if( IPS_DEBUG )
		{
			if( !isset( self::$templatesUsed['calls'][$tpl_basename] ) )
			{
				self::$templatesUsed['calls'][$tpl_basename] = 0;
			}
			self::$templatesUsed[$type][$tpl_basename] = $tpl_basename ;
			self::$templatesUsed['calls'][$tpl_basename]++;
		}
	}
	/**
	...
	*/
	...
	protected function compileFile( $tpl_basename, $tpl_basedir, $tpl_filename, $cache_dir, $compiled_filename ){
		...
		// fix IPS
		$template_compiled = App::minify( $template_compiled );
		...
		//write compiled file
		@file_put_contents( $compiled_filename, Translate::getInstance()->translate( $template_compiled ), LOCK_EX );
	}

	/**
	 * Compile template to find includes
	 * @access protected
	 */
	protected function compileInclude( $template_code ){
		
		preg_match_all( '/\{include="([^"]*)"\}/', $template_code, $code );
		
		if( !empty( $code[1] ) )
		{
			foreach( $code[1] as $tpl )
			{
				$inlude_tpl = self::$tpl_dir . $tpl;
				
				if( !file_exists( self::$tpl_dir . '/' . $tpl ) )
				{
					if( !file_exists( ABS_PATH . '/templates/__common/' . $tpl ) )
					{
						continue;
					}
					
					$inlude_tpl = ABS_PATH . '/templates/__common/' . $tpl;
				}
				
				$template_code = preg_replace( '@{include="' . $tpl . '"}@', $this->compileInclude( file_get_contents( $inlude_tpl ) ), $template_code );
			}
		}
	...
	protected function compileTemplate( $template_code, $tpl_basedir ){
		...
		//includes in tpl
		$template_code = $this->compileInclude( $template_code );
	...
	protected function compileCode( $parsed_code ){
        ...
        $loop_level = 0;
	 	...
	 	while( $html = array_shift( $parsed_code ) ){
			...
			//hook
			elseif( preg_match( '/\{hook="(\w*)(.*?)"\}/', $html, $code ) ){

				//function
				$hook = $code[ 1 ];

				//if code
				$compiled_code .=   "<?php echo do_action( '" . $hook . "'); ?>";
			}
	 		//close no parse tag
	...
	function var_replace( $html, $tag_left_delimiter, $tag_right_delimiter, $php_left_delimiter = null, $php_right_delimiter = null, $loop_level = null, $echo = null ){
		...
		if( preg_match_all( '/' . $tag_left_delimiter . '\$(\w+(?:\.\${0,1}[A-Za-z0-9_]+)*(?:(?:\[\${0,1}[A-Za-z0-9_]+\])|(?:\-\>\${0,1}[A-Za-z0-9_]+))*)(.*?)' . $tag_right_delimiter . '/', $html, $matches ) ){
                    ...
                    foreach( $parsed as $tag => $array ){
							...
							/* 
							if( $php_left_delimiter != null && $php_right_delimiter != null && !IPS_DEBUG )
							{
								$php_var = ' isset( ' . $php_var . ' ) ? ' . $php_var . ' : ' . ( IPS_DEBUG ? 'ips_log( "Not isset var ' . $php_var . '" )' : 'null' ) . ' ';
							} 
							*/
