...
class Benchmark_Timer
{
    ...
    function setMarker($name) 
    {
		...
		$this->markersMemory[$name] = memory_get_usage();
    }
    ...
    function getProfiling() 
    {
		...
		$tempMem = reset($this->markersMemory);
        ...
        foreach ($this->markers as $marker => $time) {
			...
			$result[$i]['memory_usage'] = $this->markersMemory[$marker] - $tempMem;	
            ...
            $temp = $time;
			$tempMem = $this->markersMemory[$marker];
            $i++;
        }
    ...
    function getOutput($showTotal = false, $format = 'auto') 
    {
        ...
        if ($format == 'html') {
			...
			$out .= '<td align="center"><b>Memory</b></td>';
