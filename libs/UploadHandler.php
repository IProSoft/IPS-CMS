...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null)
	{
		$this->options = array(
            ...
            // the redirect parameter, e.g. '/upload/tmp/'.
	...
	{
		if( isset( $options['file_path_ips'] ) )
		{
			$new_file_path = $options['file_path_save'];
			$file_path = $options['file_path_ips'];
		}
		else
		{
			$file_path = $this->get_upload_path($file_name);
		...
		$new_file_path = str_replace( array('\\\\', '//'), array('\\', '/'), $new_file_path );
    ...
    protected function validate($uploaded_file, $file, $error, $index) {
		...
		if ( isset( $this->options['accept_file_types_contents'] ) && !in_array( $file->type, $this->options['accept_file_types_contents'])) {
            $file->error = $this->get_error_message('accept_file_types');
            return false;
        }
    ...
    protected function trim_file_name($name,
            $type = null, $index = null, $content_range = null) {
		if( isset( $this->options['overwrite_file_name'] ) )
		{
			return $this->options['overwrite_file_name'];
		}
        ...
        $name = $this->newFileName( $ext );
        
        
        return $name;
    }
	public function newFileName( $ext )
	{
		$file_name = substr( base_convert(sha1(uniqid(mt_rand()) ), 16, 36), 0, 10);
		if( file_exists( IMG_PATH_LARGE . '/' . $file_name . $ext ) )
		{
			return $this->newFileName( $ext );
		}
    ...
    protected function orient_image($file_path) {
        ...
        $success = imagejpeg( $image, $file_path, Config::get('jpeg_compress') );
        // Free up memory (imagedestroy does not delete files):
        imagedestroy($image);
        return $success;
    }
    ...
    public function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null, $abort_check = false ) {
        $file = new stdClass();
		...
		if( $abort_check )
		{
			$file->name = $abort_check;
			$file_path = $this->get_upload_path( $file->name );
			
			$this->handle_image_file($file_path, $file);
		}
        elseif ( $this->validate($uploaded_file, $file, $error, $index))
		{
