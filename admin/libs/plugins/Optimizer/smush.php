...
function _smush($obj, $dest) {
	...
	if ( isset($s->error) && !empty($s->error) ) {
		return array(
			'error' => 'Smush error3: ' . print_r($s, true)
		);
		return false;
	}
	...
	if ( !isset($s->percent) || !isset($s->src_size) ) {
		return array(
			'error' => 'Smush error4: ' . print_r($s, true)
		);
		return false;
	}
	...
	if ($obj->error || !isset($obj->source) || empty( $obj->source ) ) {
		fclose($handle);
		return array(
			'error' => 'Fetch error'
		);
